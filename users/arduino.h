...
public:     //méthodes de la classe Arduino
    Arduino();
    int connect_arduino(const QString& com_port_name); // permet de connecter le PC à Arduino
    int close_arduino(); // permet de femer la connexion
    int write_to_arduino(const QStringList& dataList);
    int write_to_arduino_old( QByteArray d);

    QByteArray read_from_arduino();  //recevoir des données de la carte Arduino
    QSerialPort* getserial();   // accesseur
    QString getarduino_port_name();
    bool check(QString data,QString *nom,QString *prenom);
    void inserer( QString empreint , int idu);
    void update(QString currpos);
